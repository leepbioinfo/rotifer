...
def parse_cli():
    ...
    parser.add( long_arg = '--nucleotide',
                short_arg = '-n',
                dest = 'nucleotide',
                helper = 'some help',
                action = "store"
...
try:
    find_gbks = [x for x in open(args.asm).read().splitlines() if x not in ls_gbks_parsed]

except:
    find_gbks = []

try:
    find_nuc = open(args.nucleotide).read().splitlines()
except:
    find_nuc = []
...
n = len(ls_gbks)//int(7) if len(ls_gbks)//int(7) > 0 else 1
