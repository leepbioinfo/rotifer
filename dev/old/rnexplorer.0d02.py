...
(example: -y assembly, -y classification -y seq_type)',
    ...
    parser.add_argument('--exclude_by_type', '--xt',
                        ...
                        dest = 'excludebytype',
                        default = [],
                        help = 'Exclude individual features by feature type. (e.g. tRNA)',
                        action = 'append',
                        type = str)
...
if __name__ == '__main__':
    ...
    if 'ORGANISM' in to_open[0]:
        input_format_type = 'gi2operon'

    else:
        input_format_type = 'table'
    if args.outformat:
        outformat = args.outformat
    else:
        outformat = 'table' if input_format_type == 'gi2operon' else 'gi2operon' 
    if input_format_type == 'table':
        # print(to_open)
        ...
        for i, group in g:
            group = group.fillna('.')
            gi2operon(group, other_info) # print gi2operon format

    if input_format_type == 'gi2operon':
#        print(to_open)
        table(to_open) # print table format
