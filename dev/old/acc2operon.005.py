...
#    print(sub_df_all)
    ...
    for i,val in sub_groups:
        ...
        old_new_acc = val
        df = pd.DataFrame()
        df = features2df(gbk, df, db = db, accs = sub_accs, pair_acc = old_new_acc)
        if not df.empty:
            block_id = intervals(df, above, below, asm = gbk, block_id = block_id, of = outformat, pair_acc = old_new_acc)
