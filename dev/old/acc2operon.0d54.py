...
def acc2asm2gbk(accs, db, verbose, df_accs, threads = 5, api_key = '', table_name = ''):
    ...
    ### EPOST not returning the right stuf
    ### Strategy:
    ### - Check which acc returned true (some of them will not have a ipg)
    ### - Map these acc to ids
    ### - Get missing run efetch
    try:
        try:
        ...
        for sub in subs:
            if verbose:
                w +=1
                verbose_msg(f'There are {tot -w} epost left, total is {tot}')
            ...
            # The accessions with missing values
            # print(df_ipgs['original'])
            ...
            if missing:
                if verbose:
                    verbose_msg('Getting missing accession')
                    verbose_msg(f'Total missing accession: {len(missing)}')
                try:
                ...
                while tries < 10:
                    ...
                    try:
                        ...
                        for missing_acc in missing:
                            if verbose:
                                verbose_msg(f'Missing {missing_acc}')
                            ...
                            if df_ipgs.empty:
                                ...
                                if _['id'].unique()[0] not in df_ipgs['id'].unique():
                                    ...
                                    if verbose:
                                        verbose_msg(f'This {missing_acc} is in the df_ipgs already')
...
def intervals(accs = '', above = 3, below = 3,
    ...
    max_index, min_index = conn.execute(f"""SELECT
                                        max(feature_order), min(feature_order) from genomes2
                                where nuc_asm = '{nuc_asm}' and nucleotide = '{nucleotide}' and
                                type = 'CDS'""").fetchone()
