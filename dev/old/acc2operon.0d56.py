...
def _multiple_search(subs, verbose, ls, position, threads):
    ...
    try:
        ...
        # //get ipgs
            ...
            # jobs = []
            asm2ftp(find_gbks, gbks_folder, verbose= True)
            ...
            #     p = Process(target = asm2ftp, args = (sub_gbks[x], gbks_folder, verbose))
            #     jobs.append(p)
            #     p.start()
            #
            # try:
...
def acc2asm2gbk(accs, db, verbose, df_accs, threads = 5, api_key = '', table_name = ''):
    ...
    for x in range(0,len(sub_list)):
        ...
        p = Process(target = _multiple_search, args = (sub_list[x], verbose, epost_list, x, threads))
        p.start()
        jobs.append(p)
