...
def add2db(s, fi):
    ...
    try:
        ...
        with open(os.path.join(db_local_path, fi+'.config'), 'w') as yaml_file:
            yaml.dump(db_local, yaml_file, default_flow_style=False)
    print('## Added to database {0}: {1}'.format(k,v))
    print()
    show(fi, print_all = False)
    sys.exit(0)
...
def remove2db(s,fi):
    ...
    try:
        dc = {'database':{}}
        db_local = yaml.load(open(os.path.join(db_local_path, fi+'.config')))
        if s in db_local['database'].keys():
            for k,v in db_local['database'].items():
                if k != s:
                    dc['database'][k] = v
            with open(os.path.join(db_local_path, fi+'.config'), 'w') as yaml_file:
                yaml.dump(dc, yaml_file, default_flow_style=False)
            print('## Removed from database {0}'.format(s))
            print()
            show(fi, print_all = False)
        else:
    ...
    try:
        if args.func == 'pfetch':
            if args.show:
                show(args.func)
            if args.add:
                add2db(args.add, args.func)
            if args.remove:
                remove2db(args.remove, args.func)
            else:
