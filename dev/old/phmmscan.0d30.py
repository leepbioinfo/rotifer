...
if __name__ == '__main__':
    ...
    if 'parse' in outputargs.keys():
        ...
        for sub in sub_accs:
            ...
            acc_not_in_db = [y for y in sub if y not in rcf._flatten([list(x) for x in engine.execute(f'SELECT DISTINCT sequence from Pfam_32_0 where sequence in ({accs_formated}) ORDER BY sequence')])
                         ]
