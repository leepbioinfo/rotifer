...
def parse_cli():
    ...
    parser.add_argument('--api_key',
                        help = 'Insert a valid NCBI API key or a valid config file username',
                        default = '',
                        required = False)
...
def acc2asm2gbk(accs, db, verbose, df_accs, threads = 5, api_key = ''):
    ...
    accs_folder = os.path.join(db, 'acc')
    ...
    for sub in subs:
        ...
        name = str(df_accs_all['ids'].unique()).replace('[', '').replace(']', '').replace("'",'').replace('"', '').replace(' ','')
        name2save = name[0:20] if len(name) >=20 else name
        check = False
        name2 = name2save
        x = 0
        while not check:
            if name2 in os.listdir(os.path.join(accs_folder)):
                name2 = name2save+'_'+str(x)
                x+=1
            else:
                check = True
                name2save = name2
        df_accs_all.to_csv(os.path.join(accs_folder, name2save),
                           sep = '\t', index = None)
...
#        sys.exit()
        ...
        got_gbks = [x.replace('.gbff', '').replace('.tsv','') for x in os.listdir(gbks_folder)]
...
def find_common_acc(gbk, db):
    t = 2
    all_fi = []
    for f in os.listdir(os.path.join(db, 'acc')):
        all_fi.append(f)
    length = len(all_fi) // t if len(all_fi) //t >0 else 1
    sub_fi = [all_fi[x:x+length] for x in range(0, len(all_fi), length)]

    jobs = []
    pipe_list = []

    for x in range(len(sub_fi)):
        recv_end, send_end = Pipe(False)
        p = Process(target = _multiple_find_acc, args = (sub_fi[x],gbk, db, send_end))
    ...
    accs = [y for x in result_list for y in x]

    return accs

def features2df(gbk, df, db = '', pair_acc = ''):
    '''
    Parameters:
...
#    sys.stderr.write(gbk+'\n')
    ...
    accs = list(set(find_common_acc(gbk, db)))
    ...
    try:
        df = df.fillna('.')
        df.to_csv(os.path.join(db,'gbff')+'/'+gbk+'.tsv', index = None, sep = '\t')
        # delete gbk
        os.remove(os.path.join(db, 'gbff') + '/' +gbk + '.gbff')
        return (df.gbk.values, df)
    except:
        return (0,0)

def _multi_ft2df(ls_gbks = [], db = ''):
    for gbk in ls_gbks:
        df = pd.DataFrame()
        features2df(gbk, df, db = db)
...
if __name__ == '__main__':
    ...
    ls_gbks_parsed = [x.replace('.tsv', '') for x in os.listdir(os.path.join(db,'gbff')) if x.endswith('.tsv') ]

    jobs = []

    ls_gbks = [x for x in ls_gbks if x not in ls_gbks_parsed]
    n = len(ls_gbks)//args.threads if len(ls_gbks)//args.threads > 0 else 1
    sub_ls_gbks = [ls_gbks[x:x+n] for x in range(0,len(ls_gbks), n)]
    for x in range(len(sub_ls_gbks)):
        p = Process(target = _multi_ft2df, args = (sub_ls_gbks[x], db))
        jobs.append(p)
        p.start()
    try:
        for p in jobs:
    ...
    # Groupby assembly this asm is not the real assembly, this is the gbk name or a nucleotide name
    # Depends if it was possible to find a genbank
    ...
    for i,val in sub_groups:
        ...
        if verbose:
            verbose_msg('Getting neighborhoods from {0}'.format(gbk))

        sub_accs = val['acc'].values
        old_new_acc = val

        df = pd.read_csv(os.path.join(db,'gbff')+'/'+gbk+'.tsv',
                         sep = '\t')
        df = df.fillna('.')
        try:
            nucleotide = df['gbk'].values
        except:
            nucleotide = 0

        if not isinstance(nucleotide, int):
        ...
        if verbose:
            verbose_msg('Remaining assemblies: {0}'.format(str(tot_group -left_group)))
