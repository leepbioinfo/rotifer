...
Rotifer::Parser::genbank2hashes - wrapper around Bio::SeqIO for
                                  extraction of Genbank features

=head2 SYNOPSIS

 my $parser = Rotifer::Parser->create(format => "arch");
...
Rotifer::Parser::genbank2hashes is a parser baed on Bioperl
(Bio::SeqIO::RichSeqI) that extracts sequence annotations to
...
package Rotifer::Parser::genbank2hashes;
...
sub parse {
    ...
    my %keys  = ();
    my @table = ();
    my @base  = qw(accession locus);
    my @fbase = qw(primary_tag start end strand);
    my $IncludeAllTags = scalar(grep { /^all$/i } @{$self->include});
    foreach my $file (@_) {
	# Open input file
	my @file = ();
	if (blessed($file) && $file->isa("IO::Handle")) {
	    @file = (-fh => $file);
	...
	while (my $seq = $in->next_seq) {
	    ...
	    $hash->{accession} .= "." . $seq->version() if (defined $seq->version());

	    # Sequence you want annotations...
	    if (scalar @{$self->annotation}) {
		# Sequence properties
		foreach my $prop (@{$self->annotation}) {
		    $hash->{$prop} = $seq->$prop() if ($seq->can($prop));
		}

		# Taxonomy
		if (defined $seq->species && exists $annotation{"taxonomy"}) {
		    my %gettag = (
		...
		foreach my $ann ($seq->annotation->flatten_Annotations) {
			...
			next unless (exists $annotation{PMID});
			$tag   = "PMID";
