...
class SimpleParallelProcessCursor(BaseCursor):
    ...
    def _getids(self, obj):
        """
        Extract inut accessions from objects generated by _parser().
    ...
    def __getitem__(self, accession, *args, **kwargs):
        ...
        for attempt in range(0,self.tries):
            try:
                stream = self._fetcher(accession, *args, **kwargs)
            except RuntimeError:
                ...
                continue
            try:
                obj = self._parser(stream, accession, *args, **kwargs)
            except:
        ...
        if not obj:
            ...
            self.missing = self.missing.union(accession)

        return obj

    def _parser(self, stream, accession, *args, **kwargs):
        """
        Create instances of objects by processing the input
        stream generated by _fetcher().

        This method is called by __getitem__().
        """
        return NotImplementedError

    def _fetcher(self, accession, *args, **kwargs):
        """
    ...
    def _worker(self, accessions, *args, **kwargs):
        """
        Single-threaded method that will process the data
        batches generated by _splitter().
    ...
    def _splitter(self, accessions, *args, **kwargs):
        """
        ...
        processed by _worker() in each parallel batch.
        """
    ...
    def fetch_each(self, accessions, *args, **kwargs):
        ...
        with ProcessPoolExecutor(max_workers=self.threads) as executor:
            ...
            tasks = []
            for chunk in self._splitter(list(accessions), *args, **kwargs):
